...
static int exfat_count_used_clusters(unsigned char *bitmap,
...
int exfat_show_ondisk_all_info(struct exfat_blk_dev *bd)
{
	...
	root_clu_off = exfat_calc_cluster_offset(bd, clu_offset, root_clu);
	...
	bitmap_clu_off = exfat_calc_cluster_offset(bd, clu_offset,
			bitmap_clu);
	...
	exfat_info("Total size(sectors): \t\t\t%llu\n",
			((unsigned long long)total_clus*bd->cluster_size)/bd->sector_size);
	exfat_info("Available(sectors): \t\t\t%llu\n",
			((unsigned long long)(total_clus-used_clus)*bd->cluster_size)/
			 bd->sector_size);
	exfat_info("Sector size:  \t\t\t\t%u\n", bd->sector_size);
